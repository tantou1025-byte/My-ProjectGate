...
> ## ⚠️ 最初に読む：実行トリガーとブランチ運用の2大ルール
>
> ### ルール1: ドキュメントは自動実行されない
> クラウドセッションがGitHubにドキュメントをpushしても、ローカルのClaude Codeは
> それを自動では読まない。**両者をつなぐのはユーザーの手動操作だけ**。
> ローカルに実行させたい指示は、ユーザーがローカルのClaude Codeに貼り付けて初めて動く。
> クラウド側は「委譲完了」ではなく「委譲待ち（ユーザーの貼り付け待ち）」と報告すること。
>
> ### ルール2: TsukimonoTan関連のブランチは1本のみ
> 使用ブランチ：`claude/tsukimono-demo-scene-j6seey`（このブランチ）
> 過去にセッションごとに別ブランチ（opening-scene-assets-3n6cd6 等）が作られ、
> 成果物が分散するトラブルが起きた。新セッションは新ブランチを作らず、
> このブランチで作業を継続すること。
> ※未統合の別ブランチ2本が存在する：
>   - `claude/tsukimono-tan-unity-khecbg` — SaveSystem.cs等をAssets/Scripts/Core（ProjectGate側の領域）に追加。TsukimonoTanと無関係の可能性があり要ユーザー判断
>   - `claude/new-session-w2vx0s` — WebGLビルド用エディタスクリプト。WebGLはリリース後検討の方針のため保留
